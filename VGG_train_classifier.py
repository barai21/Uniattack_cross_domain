...
def extract_features(model, dataloader):
    """Extracts features from the frozen VGG backbone."""
    model.eval()
    features = []
    labels_list = []
    
    # Temporarily replace the final classification layer (index 6 in VGG) with Identity
    original_fc = model.classifier[6]
    ...
    model.classifier[6] = original_fc # Restore the classification layer
...
def run_protocol(protocol_name):
    ...
    # Replace the final fully connected layer in VGG (index 6 of the classifier)
    ...
    optimizer = optim.Adam(model.classifier[6].parameters(), lr=Config.LR, weight_decay=1e-5)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.5)

    best_dev_acer = float('inf')
    best_dev_threshold = 0.5
    model_save_path = f"best_vgg16_{protocol_name}.pth"

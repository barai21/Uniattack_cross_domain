...
class Config:
    ...
    # TODO: Download I-JEPA weights from Meta's GitHub and set the path here
    JEPA_WEIGHTS_PATH = "/home/nayana/Models/ijepa_vit_b_16.pth.tar" 
    ...
    EPOCHS = 50
    LR = 1e-4
    IMG_SIZE = 224  # JEPA (ViT-B/16) uses 224x224
...
# ==========================================
# 4. JEPA Weight Loader & Feature Extraction
# ==========================================
def load_jepa_weights(model, checkpoint_path):
    """Loads Meta's I-JEPA weights into a standard torchvision ViT."""
    if not os.path.exists(checkpoint_path):
        print(f"\n[WARNING] JEPA weights not found at {checkpoint_path}!")
        print("Using random initialization. Please download weights from Meta's I-JEPA repo.\n")
        return model
        
    print(f"\nLoading JEPA weights from {checkpoint_path}...")
    checkpoint = torch.load(checkpoint_path, map_location='cpu')
    
    # I-JEPA checkpoints usually store the encoder weights under 'encoder' or 'target_encoder'
    ...
    # Clean keys to match torchvision ViT format
    new_state_dict = {}
    for k, v in state_dict.items():
        new_k = k.replace('module.', '')
        # Meta's code sometimes names the patch embedder differently
        new_k = new_k.replace('patch_embed.proj', 'conv_proj')
        new_state_dict[new_k] = v
        
    # Load with strict=False because the classification head won't be in the JEPA checkpoint
    model.load_state_dict(new_state_dict, strict=False)
    print("Successfully loaded JEPA pre-trained weights.\n")
    return model
...
def extract_features(model, dataloader):
    """Extracts features from the frozen JEPA (ViT) backbone."""
    model.eval()
    features = []
    labels_list = []
    
    # Temporarily replace the final classification head with Identity
...
def run_protocol(protocol_name):
    ...
    # Model Setup (ViT-B/16 architecture for JEPA)
    # We do NOT load ImageNet weights, we initialize empty and load JEPA weights
    model = models.vit_b_16(weights=None)
    model = load_jepa_weights(model, Config.JEPA_WEIGHTS_PATH)
    
    # FREEZE ALL LAYERS (Self-Supervised Encoder)
    for param in model.parameters():
        param.requires_grad = False
        
    # Replace the final fully connected layer (model.heads.head)
    ...
    model_save_path = f"best_jepa_{protocol_name}.pth"
...
def main():
    ...
    for clf in classifiers:
        ...
        print("\n\n" + "="*80)
        print(f"{f'FINAL RESULTS: PROTOCOL 2 - Classifier: {clf} (I-JEPA)':^80}")
        print("="*80)

...
def evaluate(model, dataloader, criterion, threshold=None):
    ...
    # Returning labels and scores as well for ROC plotting
    return epoch_loss, acc, roc_auc, eer, apcer, bpcer, acer, calc_thresh, all_labels, all_scores

def extract_features(model, dataloader):
    """Extracts features from the frozen ResNet backbone."""
    model.eval()
    features = []
    labels_list = []
    
    # Temporarily replace the classification layer with Identity to get raw features
    ...
    model.fc = original_fc # Restore the classification layer
...
def run_protocol(protocol_name):
    ...
    optimizer = optim.Adam(model.fc.parameters(), lr=Config.LR, weight_decay=1e-5)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.5)

    best_dev_acer = float('inf')
    best_dev_threshold = 0.5
    model_save_path = f"best_resnet50_{protocol_name}.pth"
